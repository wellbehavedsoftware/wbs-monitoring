...
fn check_md_raid (host_name: &str) -> String {
	...
	let mut stat_file = match File::open(&stat_path) {
	    Ok(file) => { file }
	    Err(e)  => { return format!("MDSTAT-UNKNOWN: Failed to read /proc/mdstat: {}", e); }
	};
	...
	let template_route =
		"/etc/templates/".to_string () + & host_name + "-mdstat";

	let path =
		Path::new (& template_route);
	...
	let mut file = match File::open (& path) {
	    ...
	    Err (err) => {
	    	...
	    	return format! (
	    		"MDSTAT-UNKNOWN: Failed to read the template: {}",
	    		err);
	...
	let template_header_tokens: Vec <& str> =
		template_header.trim ().split (' ').collect ();

	let md_raid_header_tokens: Vec <& str> =
		md_raid_header.trim ().split (' ').collect ();

	if template_header_tokens.len () != md_raid_header_tokens.len () {

		return format! (
			"MD-RAID-CRITICAL: Header changed!\nCurrent:\n{}\nPrevious:\n{}\n",
			md_raid_header,
			template_header);
	...
	for token in template_header_tokens.iter () {
		...
		if ! template_header.contains (token) {
			...
			return format! (
				"MD-RAID-CRITICAL: {} was not present!\nNow:\n{}\nBefore:\n{}\n",
				token,
				md_raid_header,
				template_header);
	...
	let md_raid_devices_string =
		re.replace_all (& md_raid, "");

	let template_devices_string =
	...
	let md_raid_devices: Vec <& str> =
		blank_line_re.split (& md_raid_devices_string).collect ();

	let template_devices: Vec <& str> =
		blank_line_re.split (& template_devices_string).collect ();

	if template_devices.len () != md_raid_devices.len () {

		return format! (
			"MD-RAID-CRITICAL: The number of devices changed!\nCurrent:\n{}\nPrevious:\n{}\n",
			md_raid_devices_string,
			template_devices_string);
	...
	for md_raid_device in md_raid_devices.iter () {
		...
		let normalized_md_raid_device =
			space_re.replace_all (& md_raid_device, " ");
